...
OUTPUT_TOPIC = (
    "/origami/debug/"
    "cobotta_phase1d_exhaustive_fold_feasibility_results"
)
...
def diagnose_candidate(
    ...
    approach_angle_deg = candidate[
        "approach_angle_deg"
    ]

    entry_edge = candidate[
        "entry_edge"
    ]
...
def main():
    rospy.init_node(
        "cobotta_phase1d_exhaustive_fold_feasibility_diagnostic"
    )
    ...
    for r in results:
        print(
            "P0[{p0_index}] "
            "dir{direction_index} "
            "{approach_angle_deg:.1f}deg "
            "edge{entry_edge} {normal_sign} : "
