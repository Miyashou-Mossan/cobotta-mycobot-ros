...
rospy.init_node(
    "mycobot_contact_shift_scan_yminus6",
    anonymous=True
)
...
for shift_mm in np.arange(3.6, 5.01, 0.1):
    ...
    target.pose.position.x = (
        actual_pose.position.x + shift[0]
    )
    target.pose.position.y = (
        actual_pose.position.y + shift[1]
    )
    target.pose.position.z = (
        actual_pose.position.z + shift[2]
    )
