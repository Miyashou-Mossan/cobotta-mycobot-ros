...
class BranchPreservingSearch:
    ...
    def initial_state(self):
        row = self.base_rows[SEARCH_START]
    ...
    def candidate_offsets(self, node):
        ...
        for dx in [-ANGLE_STEP, 0.0, ANGLE_STEP]:
            for dy in [-ANGLE_STEP, 0.0, ANGLE_STEP]:

                x = node.x_deg + dx
                y = node.y_deg + dy

                if abs(x) > X_ANGLE_LIMIT:
                    continue

                if y < Y_ANGLE_MIN or y > Y_ANGLE_MAX:
                    continue
    ...
    def run(self):
        ...
        print("angle step   :", ANGLE_STEP, "deg")
        ...
        print()
        print("search log :", OUTPUT_CSV)
        print("best path  :", BEST_PATH_CSV)
